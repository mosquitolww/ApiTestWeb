  ...
  <div class="apiEdit">
    <el-form :inline="true" style="padding: 10px 20px -10px 10px;">
      ...
      <el-form-item prop="name" style="margin-bottom: 5px">
        <el-input v-model="apiMsgData.name" placeholder="接口名称" size="small"></el-input>
      </el-form-item>
    </el-form>
    <el-form :inline="true">
      <el-form-item label="高级功能" prop="name" labelWidth="80px" style="margin-bottom: 5px">
        <el-input v-model="apiMsgData.upFunc" placeholder="set_up_hooks" size="small"></el-input>
      </el-form-item>
      <el-form-item prop="name" style="margin-bottom: 5px">
        <el-input v-model="apiMsgData.downFunc" placeholder="set_down_hooks" size="small"></el-input>
      </el-form-item>
      <el-form-item prop="name" style="margin-bottom: 5px">
    ...
    <el-form style="margin: 0 0 0 10px">
      <el-form-item>
        <el-input
          ...
          <el-select
            v-model="apiMsgData.method"
            size="medium"
            style="width: 100px"
            slot="prepend"
            placeholder="选择请求方式">
            <el-option v-for="item in methods" :key="item" :value="item" :label="item"></el-option>
          </el-select>
          <el-button slot="append" type="primary" @click="ParamViewStatus = !ParamViewStatus">Params</el-button>
        </el-input>
        <el-button
          type="primary"
    ...
    <el-table
      ...
      <el-table-column property="key" label="Key" header-align="center" min-width="80">
        <template slot-scope="scope">
          <el-input v-model="scope.row.key" size="mini" placeholder="key"></el-input>
        </template>
      </el-table-column>
      ...
      <el-tab-pane label="Headers" name="first">
        <el-table
          ...
          <el-table-column property="value" label="Value" header-align="center" minWidth="200">
            <template slot-scope="scope">
              <el-input v-model="scope.row.value" size="mini" placeholder="value"></el-input>
            </template>
          </el-table-column>
          <el-table-column label="备注" header-align="center" minWidth="80">
            <template slot-scope="scope">
              <el-input v-model="scope.row.remark" size="mini" placeholder="备注"></el-input>
            </template>
          </el-table-column>
          ...
          <el-table-column label="type" header-align="center" width="100">
            <template slot-scope="scope">
              <el-select v-model="scope.row.param_type" size="mini">
                <el-option v-for="item in paramTypes" :key="item" :value="item"></el-option>
              </el-select>
            </template>
          </el-table-column>
          <el-table-column label="Value" header-align="center" minWidth="200">
            <template slot-scope="scope">
              <div v-if="scope.row.param_type === 'file'">
                <el-row>
                  <el-col :span="18">
                    <el-input v-model="scope.row.value" size="mini" :disabled="true"></el-input>
                  </el-col>
                  <el-col :span="2" style="padding-left:10px;">
                    <el-upload
                      ...
                      <el-button size="mini" type="primary" @click="tempNum(scope.$index)">点击上传</el-button>
                    </el-upload>
                  </el-col>
  ...
  methods: {
    ...
    formatData() {
      ...
      try {
        ...
        this.apiMsgData.jsonVariable = JSON.stringify(
          this.apiMsgData.jsonVariable,
          null,
          4
        );
        ...
        this.$confirm("服务器已存在相同名字文件，是否覆盖?", "提示", {
          ...
          .then(() => {
            ...
            this.$axios.post("/api/upload", form).then(response => {
              ...
              this.apiMsgData.variable[this.temp_num]["value"] =
                response.data["data"];
            });
          })
          .catch(() => {});
    ...
    editCopyApiMsg(apiMsgId, status) {
      this.$axios
        .post(this.$api.editAndCopyApiApi, { apiMsgId: apiMsgId })
        .then(response => {
          ...
          if (!response.data["data"]["json_variable"]) {
            ...
            this.apiMsgData.jsonVariable =
              response.data["data"]["json_variable"];
          }
          ...
          this.form.choiceUrl = this.proUrlData[this.projectId][
    ...
    saveAndRun() {
      ...
      this.addApiMsg(true).then(res => {
        ...
        if (res.data["status"] === 0) {
          ...
          this.$emit(
            "apiTest",
            [{ apiMsgId: res.data["api_msg_id"], num: "1" }],
            false
          );
        }
      });
    ...
    changeLine() {
      ...
      if (this.cell.style.height !== this.cell.scrollHeight + "px") {
        let i = parseInt(
          this.cell.style.height.substring(0, this.cell.style.height.length - 2)
        );
  ...
  watch: {
    monitorParam: {
      handler: function() {
        if (this.apiMsgData.param.length === 0) {
          ...
          this.apiMsgData.param[this.apiMsgData.param.length - 1]["key"] ||
          this.apiMsgData.param[this.apiMsgData.param.length - 1]["value"]
        ) {
          this.addTableRow("param");
        }
        let strParam = "";
        ...
        for (let i in this.apiMsgData.param) {
          if (
            parseInt(i) + 2 === this.apiMsgData.param.length &&
            this.apiMsgData.param[i].key
          ) {
            if (this.apiMsgData.param[i].value) {
              strParam +=
                this.apiMsgData.param[i].key +
                "=" +
                this.apiMsgData.param[i].value;
            } else {
        ...
        if (strParam) {
          this.apiMsgData.url =
            this.apiMsgData.url.split("?")[0] + "?" + strParam;
        } else {
    ...
    monitorVariable: {
      handler: function() {
        ...
        if (
          this.apiMsgData.variable[this.apiMsgData.variable.length - 1][
            "key"
          ] ||
          this.apiMsgData.variable[this.apiMsgData.variable.length - 1]["value"]
        ) {
          this.addTableRow("variable");
        }
      },
    ...
    monitorExtract: {
      handler: function() {
        ...
        if (
          this.apiMsgData.extract[this.apiMsgData.extract.length - 1]["key"] ||
          this.apiMsgData.extract[this.apiMsgData.extract.length - 1]["value"]
        ) {
          this.addTableRow("extract");
        }
      },
    ...
    monitorValidate: {
      handler: function() {
        ...
        if (
          this.apiMsgData.validate[this.apiMsgData.validate.length - 1][
            "key"
          ] ||
          this.apiMsgData.validate[this.apiMsgData.validate.length - 1]["value"]
        ) {
          this.addTableRow("validate");
        }
      },
