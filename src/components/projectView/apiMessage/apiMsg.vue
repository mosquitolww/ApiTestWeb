<template>
  <div class="caseManage" v-loading="this.loading">
    <el-form :inline="true" class="demo-form-inline search-style" size="small">
      <el-form-item label="项目" labelWidth="80px">
        <el-select v-model="form.projectId"
          ...
          <el-option v-for="(item) in proAndIdData"
                     :key="item.id"
                     :label="item.name"
                     :value="item.id">
          </el-option>
        </el-select>
        <el-select v-model="form.configId"
                   placeholder="请选择配置"
                   clearable
                   value-key="configId"
                   style="width: 150px">
          <el-option v-for="item in configData[form.projectId]"
                     :key="item.configId"
                     :label="item.name"
                     :value="item.configId">
          </el-option>
      ...
      <el-form-item>
        <el-button type="primary"
                   icon="el-icon-search"
                   @click.native="handleCurrentChange(1)">搜索</el-button>
        <el-button type="primary"
                   @click.native="initData()">录入接口信息</el-button>
        <el-button type="primary"
                   @click.native="apiTest(apiMsgList)">测试</el-button>
        <el-button type="primary"
                   icon="el-icon-view"
                   @click.native="$refs.resultFunc.lastResult()">{{null}}</el-button>
        <!--  <el-button type="primary" @click.native="$refs.importApiFunc.initData()">导入信息</el-button>  -->
        <el-button type="primary"
                   v-if="form.configId !== null && form.configId !== '' "
                   @click.native="$refs.configEditFunc.editSceneConfig(form.configId)">配置修改</el-button>
      </el-form-item>
    </el-form>
    <el-tabs v-model="numTab" class="table_padding" @tab-click="tabChange">
      <el-tab-pane label="接口信息" name="first">
        <el-row>
          <el-col :span="5" style="border:1px solid;border-color: #ffffff rgb(234, 234, 234) #ffffff #ffffff;">
            <el-row>
              <el-col style="border:1px solid;border-color: #ffffff #ffffff rgb(234, 234, 234) #ffffff;padding:2px">
                <el-dropdown @command="moduleCommand" style="float:right;">
                  <span class="el-dropdown-link"
                        style="color: #4ae2d5">操作<i class="el-icon-arrow-down el-icon--right"></i>
                  </span>
          ...
          <el-col :span="19" style="padding-left: 5px;">
              ...
              <el-table-column label="操作"
                ...
                <template slot-scope="scope">
                  <el-button type="primary"
                             icon="el-icon-edit"
                             size="mini"
                             @click.native="editCopyApi(ApiMsgTableData[scope.$index]['apiMsgId'],'edit')">编辑</el-button>
                  <el-button type="primary"
                             icon="el-icon-tickets"
                             size="mini"
                             @click.native="editCopyApi(ApiMsgTableData[scope.$index]['apiMsgId'],'copy')">复制</el-button>
                  <el-button type="danger"
                             icon="el-icon-delete"
                             size="mini"
                             @click.native="sureView(delApi,ApiMsgTableData[scope.$index]['apiMsgId'],ApiMsgTableData[scope.$index]['name'])">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
            ...
            <el-button @click="cancelSelection()"
                       size="mini"
                       style="position: absolute;margin-top: 2px;">取消选择</el-button>
      ...
      <el-tab-pane label="接口配置"
                   name="second"
                   v-show="apiEditViewStatus"
                   style="background-color: rgb(250, 250, 250);min-height: 760px">
        <apiEdit :projectId="form.projectId"
    ...
    <el-dialog title="接口模块配置"
               :visible.sync="moduleData.viewStatus"
               width="30%">
      <el-form>
      ...
      <div slot="footer"
        ...
        <el-button size="small"
                   @click="moduleData.viewStatus = false">取 消</el-button>
        <el-button type="primary"
                   size="small"
                   @click.native="addModule()">确 定</el-button>
      </div>
    </el-dialog>

    <importApi :projectId="form.projectId"
    ...
    <configEdit :proAndIdData="proAndIdData"
...
export default {
  ...
  methods: {
    ...
    delApi(apiMsgId) {
      //  删除接口信息
      this.$axios
        .post(this.$api.delApiApi, { apiMsgId: apiMsgId })
        .then(response => {
          this.messageShow(this, response);
          this.form.apiName = null;
          if (
            (this.apiMsgPage.currentPage - 1) * this.apiMsgPage.sizePage + 1 ===
            this.apiMsgPage.total
          ) {
      ...
      this.$axios
        ...
        .then(response => {
          if (this.messageShow(this, response)) {
            ...
            this.proModelData[this.form.projectId] =
              response.data["all_module"];
    ...
    treeClick(data) {
      ...
      let index = this.moduleDataList
        .map(item => item.moduleId)
        .indexOf(data["moduleId"]); //  获取当前节点的下标
    ...
    delModule() {
      //  删除模块
      this.$axios
        .post(this.$api.delModuleApi, { id: this.form.module.moduleId })
        .then(response => {
          this.messageShow(this, response);
          this.moduleData.name = "";
          if (
            (this.modulePage.currentPage - 1) * this.modulePage.sizePage + 1 ===
            this.modulePage.total
          ) {
